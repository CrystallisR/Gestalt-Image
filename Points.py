...
'''
def drawPoints(path, num=2, img_sz=(1, 1), dpi=128, psz=10, colors=[]):
    ...
    plt.scatter(x_list1, y_list1, s=psz, c=colors[random.randint(0, c_num-1)])
    plt.scatter(x_list2, y_list2, s=psz, c=colors[random.randint(0, c_num-1)])
    if num > 1:
        for _ in range(num-2):
            fg = True if random.randint(0,1) == 1 else False
            x_list, y_list = setPoints(fg)
            plt.scatter(x_list, y_list, s=psz, c=colors[random.randint(0, c_num-1)])
    plt.axis('off')
    plt.savefig(path)
...
# for display purpose
def drawPointsMul(path, num_range=(2, 5), plots=(5, 5), img_sz=(1, 1), dpi=128, psz=10, colors=[]):
    if colors is None: colors = ["black"]
    c_num = len(colors)
